...
函式回傳: {"product_name": "深綠色V領棉質襯衫", "order_date": "2026-01-15", ...}
Gemini: "您在 1 月 15 日購買了深綠色V領棉質襯衫，金額 NT$890。"
```

回答完全正確，但總覺得少了什麼——用戶說的是「那件綠色襯衫」，Gemini 只是轉述 JSON 裡的文字，完全沒有辦法「確認」那件衣服長什麼樣子。如果資料庫裡剛好有三件不同款的綠色上衣，AI 根本無法判斷哪件才是用戶記憶中的那件。
...
> 用戶：「幫我看看我上個月買的那件綠色襯衫」
> Bot（傳統）：「您購買了深綠色V領棉質襯衫。」
> Bot（Multimodal）：「圖片中是一件深綠色的 V 領款式棉質襯衫，布料看起來輕薄透氣。這是您 1 月 15 日的訂單 ORD-2026-0115，共 NT$890，已送達。」+ **商品圖片**
...
這個 demo 不依賴外部圖床，用 **Pillow** 即時生成商品圖片（背景色對應商品顏色，印上商品名稱和價格），啟動時無需任何圖片素材：

```python
def generate_product_image(product: dict) -> bytes:
    img = Image.new("RGB", (400, 400), color=product["bg_color"])
...
產生的圖片（JPEG bytes）有兩個用途：
1. 作為 `FunctionResponseBlob` 夾帶給 Gemini 分析
...
解法是在 FastAPI 加一個 `/images/{image_id}` 端點，把 Pillow 生成的圖片暫存在 `image_cache` dict，LINE 透過這個端點取圖：
...
系統內建 5 件商品，每位 LINE 用戶第一次查詢訂單時，自動綁定兩筆 demo 訂單：

| 訂單編號 | 日期 | 商品 |
|---------|------|------|
| ORD-2026-0115 | 2026-01-15 | P001 深綠色V領棉質襯衫 |
| ORD-2026-0108 | 2026-01-08 | P003 深藍色直筒牛仔褲 |

### 場景 1：「幫我看看我上個月買的那件綠色襯衫」

```
用戶傳送: "幫我看看我上個月買的那件綠色襯衫"

[Gemini → function_call]
  get_order_history(time_range="last_3_months")

[_execute_tool 執行]
  - get_order_history() 回傳兩筆訂單
  - generate_product_image(P001) → 深綠色背景 JPEG bytes

[Multimodal Function Response]
  Part.from_function_response(
    name="get_order_history",
    response={"orders": [...], "order_count": 2},
    parts=[FunctionResponsePart(inline_data=FunctionResponseBlob(data=<圖片>))]
  )

[Gemini 看到圖片後回應]
  "圖片中是一件深綠色的 V 領款式棉質上衣，整體顏色飽和，
   布料看起來輕薄舒適。這是您 2026 年 1 月 15 日的訂單
   ORD-2026-0115，共 NT$890，狀態：已送達。"
...
[Gemini 看到 P004 粉紅色格紋洋裝圖片後]
  "有的！圖片中這件粉紅色格紋洋裝（P004）採 A 字裙擺，
   格紋圖案精緻浪漫，售價 NT$1,590，目前還有 5 件庫存。"
```

### 場景 3：「P003 那件牛仔褲的詳細規格」
...
[Gemini 看到深藍色牛仔褲圖片後]
  "圖片中是一件深藍色直筒牛仔褲（P003），顏色深沉穩重，
   採用彈性牛仔布料，直筒版型適合各種場合，售價 NT$1,290。"
```
...
| 回答品質 | 「您購買了深綠色V領棉質襯衫」 | 「圖片中是一件深綠色 V 領款式...」|
